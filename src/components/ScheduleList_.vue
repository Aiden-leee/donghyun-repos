							...
							>
								<ul class="schedule_box">
									<template v-if="scheduleList.length > 0">
										...
										<div
											class="li"
											v-for="(list, index) in scheduleList"
											:key="index"
										>
											<!-- 강의 시간표 요일 -->
											<div
												class="s_days"
												v-for="(cell, cell_index) in list.options.day_index"
												:key="cell_index"
											>
												<!-- 각 요일에 해당하는 인덱스 조건 -->
												<template v-if="cell == idx">
...
export default {
	...
	computed: {
		...
		scheduleList() {
			// 등록된 강의 목록
			this.$store.state.schedule.forEach(function(item) {
				let options = {};
				options.day_index = [];
				let range_time = item.end_time - item.start_time;
				let start = item.start_time;
				let days = ['월', '화', '수', '목', '금'];
				let day = item.dayofweek.split('');
				let code_no = item.code.split('-');
